 ...
 * lastCell="图片右下角单元格坐标，左上角坐为指令所在单元格" [,imageType="JPG"] [,size="auto |
...
public class ImageCommand extends AbstractCommand {
	...
	public Size applyAt(CellRef cellRef, Context context) {
		...
		try {
			...
			//	throw new IllegalArgumentException("出现异常，终止生成excel", ex);
			//}
		}
		// 恢复原有的样式
					...
					if (!JxlsUtil.me().isAbsolutePath(imgSrc)) {
						if (imageRoot.startsWith("classpath:")) {
							...
							InputStream resourceAsStream = JxlsBuilder.class
									.getResourceAsStream(templateRoot + File.separator + imgSrc);
