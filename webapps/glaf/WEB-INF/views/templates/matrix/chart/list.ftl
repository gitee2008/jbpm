...
<script type="text/javascript">
		...
		jQuery(function(){
			jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'${request.contextPath}/matrix/chart/json',
				remoteSort: false,
				singleSelect:true,
				idField:'id',
				pageSize:20,
		...
		function editRow(id){
			var link = "${request.contextPath}/matrix/chart/edit?chartId="+id;
				jQuery.layer({
					type: 2,
					maxmin: true,
					shadeClose: true,
		...
		function viewChart(id){
			var link = "${request.contextPath}/matrix/chart/showChart?chartId="+id;
				jQuery.layer({
					type: 2,
					maxmin: true,
					shadeClose: true,
		...
		function onRowClick(rowIndex, row){
           // window.open('${request.contextPath}/matrix/chart/edit?chartId='+row.id);
		   editRow(row.id);
		}
		...
		function create(){	
			location.href="${request.contextPath}/matrix/chart/edit?nodeId=0";
		}
		...
		function editSelected(){
			...
			if (selected ){
			    //location.href="${request.contextPath}/matrix/chart/edit?chartId="+selected.id;
				var link = "${request.contextPath}/matrix/chart/edit?chartId="+selected.id;
				window.open(link);
			}
		}
		...
		function deleteSelections(){
			...
			if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			  ...
			  jQuery.ajax({
				   type: "POST",
				   url: '${request.contextPath}/rs/matrix/chart/deleteAll?chartIds='+chartIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
