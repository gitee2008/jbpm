...
<script type="text/javascript">
	...
	function getUrl(treeId, treeNode) {
		if(treeNode != null){
		    ...
		    return "${request.contextPath}/rs/matrix/chart/treeJson?selected=${selected}"+param;
		}
		return "${request.contextPath}/rs/matrix/chart/treeJson?nodeCode=${nodeCode}&selected=${selected}";
	}
	...
	function setFormData(){
		...
		if(confirm("您确定选择'"+sx+"'吗？")){
			  ...
			  <#if chooseType == 'link'>
                 x_elementId.value="/matrix/chart/showChart?chartId="+value;
			     x_element_name.value="/matrix/chart/showChart?chartId="+value;
			  </#if>
			  window.close();
		}
